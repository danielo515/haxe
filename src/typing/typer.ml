...
let unify_min ctx el =
	...
	with Error (Unify l,p,nl) ->
		if not ctx.untyped then display_error ~nesting_level:nl ctx.com (error_msg (Unify l)) p;
		(List.hd el).etype
...
and type_new ctx path el with_type force_inline p =
	...
	let unify_constructor_call c fa =
		try
			...
			fcc
		with Error (e,p,nl) ->
			typing_error ~nesting_level:nl (error_msg e) p;
	in
	...
	| _ ->
		...
		display_error ~nesting_level:nl ctx.com (error_msg err) p;
...
and type_map_declaration ctx e1 el with_type p =
	...
	let check_key e_key =
		try
			...
			typing_error ~nesting_level:1 (compl_msg "Previously defined here") p
		with Not_found ->
